...
if (!$warehouse_id || !filter_var($warehouse_id, FILTER_VALIDATE_INT)) {
    header('Location: admin_warehouses_list.php?error=' . urlencode('Cannot add location: No valid warehouse specified.'));
    exit;
}
...
if (!$warehouse) {
    header('Location: admin_warehouses_list.php?error=' . urlencode('Cannot add location: Warehouse not found.'));
    exit;
}

$page_title = "Admin - Add Stock Location to " . htmlspecialchars($warehouse['name']);
$message = '';
$message_type = '';

// For parent location dropdown
$possible_parent_locations = get_stock_locations_by_warehouse($warehouse_id);
$location_types = ['AISLE', 'SHELF', 'BIN', 'ZONE', 'PALLET', 'RECEIVING', 'SHIPPING', 'STAGING'];
sort($location_types);


if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token_add_sl'] ?? '')) {
        die('CSRF token validation failed.');
    }

    // Ensure warehouse_id from POST matches the one from GET for security/consistency
    if (($_POST['warehouse_id'] ?? null) != $warehouse_id) {
        die('Warehouse ID mismatch.');
    }
    ...
    if (empty($data['location_code'])) {
        ...
        $location_id = create_stock_location($data);
        if ($location_id) {
            $message = 'Stock Location "' . htmlspecialchars($data['location_code']) . '" added successfully!';
            $message_type = 'success';
            $_POST = []; // Clear form
        } else {
            $message = $_SESSION['error_message'] ?? 'Failed to add stock location.';
            $message_type = 'error';
            unset($_SESSION['error_message']);
        }
    }
...
if (empty($_SESSION['csrf_token_add_sl'])) {
    $_SESSION['csrf_token_add_sl'] = bin2hex(random_bytes(32));
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container">
        ...
        <form action="admin_add_stock_location.php?warehouse_id=<?php echo htmlspecialchars($warehouse_id); ?>" method="POST">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token_add_sl']); ?>">
            <input type="hidden" name="warehouse_id" value="<?php echo htmlspecialchars($warehouse_id); ?>">

            <div class="form-section">
                <div class="grid-container">
                    <div><label for="location_code">Location Code (e.g., A1-S2-B03):</label><input type="text" id="location_code" name="location_code" value="<?php echo htmlspecialchars($_POST['location_code'] ?? ''); ?>" required></div>
                    <div><label for="location_type">Location Type:</label>
                        <select id="location_type" name="location_type">
                            <option value="">-- Select Type --</option>
                            <?php foreach ($location_types as $type): ?>
                            <option value="<?php echo htmlspecialchars($type); ?>" <?php echo (($_POST['location_type'] ?? '') == $type) ? 'selected' : ''; ?>><?php echo htmlspecialchars($type); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div><label for="parent_location_id">Parent Location (Optional):</label>
                        <select id="parent_location_id" name="parent_location_id">
                            ...
                            <option value="<?php echo htmlspecialchars($pl['id']); ?>" <?php echo (($_POST['parent_location_id'] ?? null) == $pl['id']) ? 'selected' : ''; ?>>
            ...
            <div class="form-section">
                <div class="grid-container" style="margin-top:15px;">
                    <div class="checkbox-group"><label><input type="checkbox" name="is_pickable" value="1" <?php echo (isset($_POST['is_pickable']) || !$_SERVER['REQUEST_METHOD'] === 'POST') ? 'checked' : ''; ?>> Is Pickable</label></div>
                    <div class="checkbox-group"><label><input type="checkbox" name="is_sellable" value="1" <?php echo (isset($_POST['is_sellable']) || !$_SERVER['REQUEST_METHOD'] === 'POST') ? 'checked' : ''; ?>> Is Sellable (stock here counts as available)</label></div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="grid-container">
                    <div><label for="max_weight_kg">Max Weight (kg):</label><input type="number" id="max_weight_kg" name="max_weight_kg" step="0.01" value="<?php echo htmlspecialchars($_POST['max_weight_kg'] ?? ''); ?>"></div>
                    <div><label for="max_volume_m3">Max Volume (m³):</label><input type="number" id="max_volume_m3" name="max_volume_m3" step="0.001" value="<?php echo htmlspecialchars($_POST['max_volume_m3'] ?? ''); ?>"></div>
                </div>
            </div>

            <input type="submit" value="Add Stock Location">
        </form>
         <div class="nav-links">
            <a href="admin_stock_locations_list.php?warehouse_id=<?php echo htmlspecialchars($warehouse_id); ?>">Back to Locations List</a>
        </div>
    </div>
</body>
</html>
