...
$page_title = "Admin - Add New Seller";
$message = '';
$message_type = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token_add_seller'] ?? '')) {
    ...
    if (empty($data['seller_name']) || empty($data['email'])) {
        $message = 'Seller Name and Email are required.';
        $message_type = 'error';
        ...
        $seller_id = create_seller($data);
        if ($seller_id) {
            $_SESSION['flash_message'] = 'Seller "' . htmlspecialchars($data['seller_name']) . '" added successfully!';
            $_SESSION['flash_message_type'] = 'success';
            header('Location: admin_sellers_list.php');
            exit;
        } else {
            $message = $_SESSION['error_message'] ?? 'Failed to add seller.';
            $message_type = 'error';
            unset($_SESSION['error_message']);
        }
    }
...
if (empty($_SESSION['csrf_token_add_seller'])) {
    $_SESSION['csrf_token_add_seller'] = bin2hex(random_bytes(32));
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container">
        ...
        <form action="admin_add_seller.php" method="POST">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token_add_seller']); ?>">

            <div class="grid-container">
                <div><label for="seller_name">Seller Name:</label><input type="text" id="seller_name" name="seller_name" value="<?php echo htmlspecialchars($_POST['seller_name'] ?? ''); ?>" required></div>
                <div><label for="contact_person">Contact Person:</label><input type="text" id="contact_person" name="contact_person" value="<?php echo htmlspecialchars($_POST['contact_person'] ?? ''); ?>"></div>
            </div>
            <div class="grid-container">
                <div><label for="email">Email:</label><input type="email" id="email" name="email" value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>" required></div>
                <div><label for="phone">Phone:</label><input type="tel" id="phone" name="phone" value="<?php echo htmlspecialchars($_POST['phone'] ?? ''); ?>"></div>
            </div>
            <div><label for="address">Address:</label><textarea id="address" name="address"><?php echo htmlspecialchars($_POST['address'] ?? ''); ?></textarea></div>

            <div class="grid-container">
                <div><label for="commission_rate_percentage">Commission Rate (%):</label><input type="number" id="commission_rate_percentage" name="commission_rate_percentage" step="0.01" min="0" max="100" value="<?php echo htmlspecialchars($_POST['commission_rate_percentage'] ?? '0.00'); ?>"></div>
                <div><label for="user_id">Linked User Account (Optional):</label>
                    <select id="user_id" name="user_id">
                        <option value="">-- None --</option>
                        <?php foreach ($all_users as $user): ?>
                        <option value="<?php echo $user['id']; ?>" <?php echo (($_POST['user_id'] ?? null) == $user['id']) ? 'selected' : ''; ?>>
