...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_version'])) {
    ...
    if (!empty($version) && !empty($content)) {
        if (add_terms_version($content, $version)) {
            $message = 'New terms & conditions version added successfully.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['publish_version'])) {
    ...
    if ($version_id) {
        if (publish_terms_version((int)$version_id)) {
            $message = 'Terms & conditions version published successfully.';
            $message_type = 'success';
        } else {
...
$terms_versions = get_all_terms_versions();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Admin - Terms & Conditions Management</title>
    <style>
...
<body>
    <div class="container">
        ...
        <h1>Admin - Terms & Conditions Management</h1>
        ...
        <form action="admin_terms_conditions.php" method="POST">
        ...
        <?php else: ?>
            <table>
                ...
                <tbody>
                    <?php foreach ($terms_versions as $tv): ?>
                        <?php
                            // Generate a unique CSRF token for each publish form
                            $publish_csrf_token_name = 'csrf_token_publish_' . $tv['id'];
                            if (empty($_SESSION[$publish_csrf_token_name])) {
                                $_SESSION[$publish_csrf_token_name] = bin2hex(random_bytes(32));
                            }
                        ?>
                        <tr <?php echo $tv['is_published'] ? 'class="published"' : ''; ?>>
                            <td><?php echo htmlspecialchars($tv['id']); ?></td>
                            <td><?php echo htmlspecialchars($tv['version']); ?></td>
                            <td><?php echo htmlspecialchars($tv['created_at']); ?></td>
                            <td><?php echo $tv['is_published'] ? 'Yes' : 'No'; ?></td>
                            <td><?php echo htmlspecialchars($tv['published_at'] ?? 'N/A'); ?></td>
                            <td>
                                <?php if (!$tv['is_published']): ?>
                                    <form action="admin_terms_conditions.php" method="POST" class="action-form">
                                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION[$publish_csrf_token_name]); ?>">
                                        <input type="hidden" name="version_id" value="<?php echo htmlspecialchars($tv['id']); ?>">
        ...
        <?php endif; ?>
        <p><a href="terms_conditions.php" target="_blank">View Public Terms & Conditions Page</a></p>
