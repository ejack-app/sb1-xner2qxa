...
require_once __DIR__ . '/../src/user_functions.php'; // For get_all_users or a specific driver function

$page_title = "Admin - Add New Vehicle";
$message = '';
$message_type = '';

$available_drivers = get_available_drivers(); // From vehicle_functions.php (or user_functions.php)

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token_add_vehicle'] ?? '')) {
    ...
    if (empty($data['vehicle_name']) || empty($data['vehicle_type'])) {
        ...
        if ($vehicle_id) {
            $message = 'Vehicle "' . htmlspecialchars($data['vehicle_name']) . '" added successfully!';
            $message_type = 'success';
            $_POST = []; // Clear form fields on success
        } else {
            $message = $_SESSION['error_message'] ?? 'Failed to add vehicle.';
            $message_type = 'error';
            unset($_SESSION['error_message']);
        }
    }
...
if (empty($_SESSION['csrf_token_add_vehicle'])) {
    $_SESSION['csrf_token_add_vehicle'] = bin2hex(random_bytes(32));
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container">
        ...
        <form action="admin_add_vehicle.php" method="POST">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token_add_vehicle']); ?>">

            <div class="grid-container">
                <div><label for="vehicle_name">Vehicle Name/Identifier:</label><input type="text" id="vehicle_name" name="vehicle_name" value="<?php echo htmlspecialchars($_POST['vehicle_name'] ?? ''); ?>" required></div>
                <div><label for="license_plate">License Plate:</label><input type="text" id="license_plate" name="license_plate" value="<?php echo htmlspecialchars($_POST['license_plate'] ?? ''); ?>"></div>
            </div>
            <div class="grid-container">
                <div><label for="make">Make (e.g., Ford):</label><input type="text" id="make" name="make" value="<?php echo htmlspecialchars($_POST['make'] ?? ''); ?>"></div>
                <div><label for="model">Model (e.g., Transit):</label><input type="text" id="model" name="model" value="<?php echo htmlspecialchars($_POST['model'] ?? ''); ?>"></div>
                <div><label for="year">Year:</label><input type="number" id="year" name="year" min="1900" max="<?php echo date('Y') + 1; ?>" value="<?php echo htmlspecialchars($_POST['year'] ?? ''); ?>"></div>
            </div>
            <div><label for="vin_number">VIN Number:</label><input type="text" id="vin_number" name="vin_number" value="<?php echo htmlspecialchars($_POST['vin_number'] ?? ''); ?>"></div>
            ...
            <div class="grid-container">
                <div><label for="vehicle_type">Vehicle Type:</label>
                    <select id="vehicle_type" name="vehicle_type" required>
                        ...
                        <option value="<?php echo htmlspecialchars($type); ?>" <?php echo (($_POST['vehicle_type'] ?? '') === $type) ? 'selected' : ''; ?>><?php echo htmlspecialchars($type); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div><label for="status">Status:</label>
                    <select id="status" name="status" required>
                        ...
                        <option value="<?php echo htmlspecialchars($status_val); ?>" <?php echo (($_POST['status'] ?? 'Available') === $status_val) ? 'selected' : ''; ?>><?php echo htmlspecialchars($status_val); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="grid-container">
                <div><label for="current_driver_id">Current Driver (Optional):</label>
                    <select id="current_driver_id" name="current_driver_id">
                        ...
                        <option value="<?php echo htmlspecialchars($driver['id']); ?>" <?php echo (($_POST['current_driver_id'] ?? null) == $driver['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($driver['username']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="grid-container">
                <div><label for="max_payload_kg">Max Payload (kg, Optional):</label><input type="number" id="max_payload_kg" name="max_payload_kg" step="0.01" value="<?php echo htmlspecialchars($_POST['max_payload_kg'] ?? ''); ?>"></div>
                <div><label for="max_volume_m3">Max Volume (m³, Optional):</label><input type="number" id="max_volume_m3" name="max_volume_m3" step="0.001" value="<?php echo htmlspecialchars($_POST['max_volume_m3'] ?? ''); ?>"></div>
            </div>
